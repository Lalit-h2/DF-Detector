  ...
  return (
    <Layout>
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h2 className="text-3xl font-bold font-display text-white mb-2">Analysis History</h2>
          <p className="text-muted-foreground">Manage and review past video detections</p>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-40 bg-card border-white/10 text-white">
                <DropdownMenuItem onClick={() => setFilter("ALL")}>All Results</DropdownMenuItem>
                <DropdownMenuItem onClick={() => setFilter("REAL")}>Real Only</DropdownMenuItem>
                <DropdownMenuItem onClick={() => setFilter("FAKE")}>Fake Only</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      </div>
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                    <thead className="bg-white/5 border-b border-white/10">
                        <tr>
                            <th className="px-6 py-4 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Video File</th>
                            <th className="px-6 py-4 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Date</th>
                            <th className="px-6 py-4 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Result</th>
                            ...
                            <tr key={video.id} className="group hover:bg-white/5 transition-colors">
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="font-medium text-white truncate max-w-[200px]">{video.filename}</p>
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                                        ...
                                        {video.uploadDate ? format(new Date(video.uploadDate), "MMM d, yyyy") : "-"}
                                    </div>
                                </td>
                                <td className="px-6 py-4">
                                    <div className={`
                                    ...
                                    `}>
                                        {video.prediction === 'REAL' ? <ShieldCheck className="w-3 h-3" /> : <ShieldAlert className="w-3 h-3" />}
                                        {video.prediction}
                                    </div>
                                </td>
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-sm font-medium text-white">{video.confidence}%</span>
                                    </div>
                                </td>
                                        ...
                                        <DropdownMenuContent align="end" className="bg-card border-white/10 text-white">
                                            ...
                                            <AlertDialog>
                                                <AlertDialogTrigger asChild>
                                                    <DropdownMenuItem onSelect={(e) => e.preventDefault()} className="text-red-400 hover:text-red-300 hover:bg-red-500/10 cursor-pointer">
                                                ...
                                                <AlertDialogContent className="bg-card border-white/10 text-white">
                                                    <AlertDialogHeader>
                                                        ...
                                                        <AlertDialogDescription className="text-muted-foreground">
                                                            This action cannot be undone. This will permanently delete the video analysis record.
                                                        </AlertDialogDescription>
                                                    </AlertDialogHeader>
                                                    <AlertDialogFooter>
