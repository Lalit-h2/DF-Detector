...
export default function ResultsPage() {
  ...
  const handleDownload = async () => {
    try {
      ...
      link.download = `${video.filename}-report.png`;
  ...
  const handleShare = async () => {
    try {
      ...
      canvas.toBlob(async (blob) => {
        ...
        const file = new File([blob], "deepfake-report.png", {
          type: "image/png",
        });
        ...
        if (
          ...
          await navigator.share({
            ...
            text: `Analysis result for ${video.filename}`,
            files: [file],
          });
        }
      });
    ...
    return (
      <Layout>
        <div className="text-center mt-20">
          ...
          <Link href="/dashboard">
            <span className="text-primary hover:underline mt-4 inline-block cursor-pointer">
              Return to Dashboard
            </span>
          </Link>
        </div>
      ...
      <div ref={reportRef}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="aspect-video bg-black/40 rounded-3xl border border-white/10 flex items-center justify-center overflow-hidden relative group">
              ...
              <div className="absolute top-4 right-4 z-20">
                <div
                  className={`px-4 py-1.5 rounded-full backdrop-blur-md border ${bgClass} ${borderClass} ${colorClass} font-bold text-sm flex items-center gap-2`}
                >
                  <div
                    className={`w-2 h-2 rounded-full ${isFake ? "bg-red-500 animate-pulse" : "bg-emerald-500"}`}
                  />
                  {video.prediction}
                </div>
              </div>
                  ...
                  <AreaChart data={timelineData}>
                    <defs>
                      ...
                      >
                        <stop
                          offset="5%"
                          stopColor={isFake ? "#ef4444" : "#10b981"}
                          stopOpacity={0.3}
                        />
                        <stop
                          offset="95%"
          ...
          <div className="space-y-6">
            ...
            >
              <div
                className={`absolute -right-10 -top-10 w-40 h-40 ${isFake ? "bg-red-500/20" : "bg-emerald-500/20"} blur-[50px] rounded-full`}
              />
              ...
              <div className="relative z-10 text-center">
                ...
                <div
                  className={`inline-block px-4 py-2 rounded-lg ${bgClass} ${colorClass} font-bold border ${borderClass}`}
                >
                  {video.riskBadge}
                </div>
              </div>
              ...
              <div className="p-4 rounded-xl bg-yellow-500/10 border border-yellow-500/20 flex gap-3 items-start">
                ...
                <div>
                  ...
                  <p className="text-xs text-yellow-200/70 leading-relaxed">
                    This content has been flagged with high confidence as
                    synthetic media. Please verify source authenticity before
                    sharing.
                  </p>
                </div>
              </div>
